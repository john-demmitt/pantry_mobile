...
class MyLogPHP {
   ...
   * There are 4 possible levels: INFO, WARNING, DEBUG, ERROR
   * @param string $value The value that will be recorded on log file.
   * @param string $tag Any possible tag to help the developer to
  ...
  /**
   * Function to write non INFOrmation messages that will
   * be written into $LOGFILENAME.
   * 
   * @param string $value
   * @param string $tag 
   */
   ...
   *
   * Warning messages are for non-fatal errors, so, the script will work
   * properly even
   * if WARNING errors appears, but this is a thing that you must
   * ponderate about.
   * 
   * @param string $value
   * @param string $tag 
   ...
   *
   * These messages are for fatal errors. Your script will NOT work
   * properly if an ERROR happens, right?
   * 
   * @param string $value
   * @param string $tag 
   */
